...
const ProductsView = () => {
  ...
  // ✅ Safe fetch function using config.url
  const fetchProducts = () => {
    console.log("Backend base URL:", config.url);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        formData.append("file", imageFile);
        const uploadRes = await axios.post(
          `${config.url}/api/products/upload`,
          formData,
          { headers: { "Content-Type": "multipart/form-data" } }
        );
        imageUrl = uploadRes.data.fileUrl || uploadRes.data;
      ...
      if (editingProduct) {
        await axios.put(`${config.url}/api/products/${editingProduct.id}`, payload);
      } else {
        await axios.post(`${config.url}/api/products`, payload);
      }
